...
CREATE DATABASE desafio_cristian_gallardo_133;

-- Ingreso a la base de datos 

\c desafio_cristian_gallardo_133
...
SELECT AVG(usuarios) AS promedio FROM (SELECT fecha, SUM (cantidad) AS usuarios FROM INSCRITOS GROUP BY fecha) AS results
-- 96.75
...
SELECT AVG(personas) FROM (SELECT SUM(cantidad) AS personas FROM inscritos WHERE fecha >= '01-03-2021' GROUP BY fecha) AS personas_tercer_dia
-- 96.75
